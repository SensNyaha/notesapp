...
export function CollaborationScreen({user,onBack}:{user:User;onBack:()=>void}){
  ...
  return e('main',{class:'settings-screen collaboration-screen'},
    e(PageHeader,{eyebrow:'Совместная работа',title:'Контакты и доступ',description:'Контакты, приглашения в shared vault и account-wide E2EE-ключ.',back:onBack}),
    ...
    e('nav',{class:'subtabs collaboration-tabs','aria-label':'Совместная работа'},
      ...
      e('button',{class:tab==='security'?'selected':'',onClick:()=>setTab('security')},e(UiIcon,{name:'key',size:18}),'E2EE-ключ')),
    ...
    tab==='contacts'&&e('div',{class:'collaboration-grid'},
      e('section',{class:'settings-panel contact-search-panel'},
        ...
        found&&e('article',{class:'contact-result'},
          ...
          e('div',null,e('strong',null,found.user.login),e('small',null,found.identity?'Fingerprint '+shortFingerprint(found.identity.fingerprint):'E2EE-ключ ещё не создан')),
      ...
      e('section',{class:'settings-panel collaboration-span'},
        e('div',{class:'settings-panel-heading'},e('div',null,e('h2',null,'Контакты'),e('p',null,'Подтверждённые пользователи и их E2EE fingerprint.'))),
        ...
        e('div',{class:'contact-list'},contacts.map(contact=>e('article',{class:'contact-row',key:contact.id},e('span',{class:'avatar'},contact.login.slice(0,1).toUpperCase()),
          e('div',null,e('strong',null,contact.login),contact.identity?e('small',{class:'mono'},shortFingerprint(contact.identity.fingerprint)):e('small',{class:'danger-text'},'E2EE-ключ недоступен')),
          e('button',{class:'text-danger-button',disabled:busy,onClick:()=>{if(confirm('Удалить '+contact.login+' из контактов? Доступ к уже расшаренным хранилищам автоматически не отзывается.'))void run(()=>removeContact(user,contact.id).then(()=>{}));}},'Удалить')))))),

    tab==='invites'&&e('section',{class:'settings-panel'},
      e('div',{class:'settings-panel-heading'},e('div',null,e('h2',null,'Приглашения в хранилища'),e('p',null,'Примите роль viewer/editor после разблокировки collaboration key.'))),
      !vaultInvites.length&&e('div',{class:'empty-state'},e(UiIcon,{name:'folder',size:30}),e('h2',null,'Приглашений нет'),e('p',null,'Новые shared vault появятся здесь.')),
      e('div',{class:'invite-list'},vaultInvites.map(item=>e('article',{class:'invite-card',key:item.id},
        ...
        e('div',{class:'row-actions'},e('button',{class:'primary',disabled:busy||!unlocked,onClick:()=>void run(async()=>{await acceptVaultInvite(user,item.id);setStatus('Приглашение принято. Выполняется синхронизация shared vault.');})},'Принять'),e('button',{class:'tertiary-button',disabled:busy,onClick:()=>void run(()=>rejectVaultInvite(user,item.id).then(()=>{}))},'Отклонить')),
        !unlocked&&e('small',{class:'invite-hint'},'Сначала откройте E2EE-ключ совместной работы.'))))),
    ...
    tab==='security'&&e('section',{class:'settings-panel collaboration-security'},
      e('div',{class:'security-key-hero'},
        ...
        e('div',null,e('h2',null,'E2EE-ключ совместной работы'),e('p',null,!identity?'Ключ ещё не создан.':('Версия '+identity.version+' · '+(unlocked?'открыт только в runtime':'заблокирован')))),
        e('span',{class:'status-pill '+(unlocked?'success':'neutral')},e(StatusDot,{tone:unlocked?'success':'neutral'}),unlocked?'Открыт':'Заблокирован')),
      e('p',{class:'settings-footnote'},'Один account-wide ECDH-ключ защищает collaboration identity. Сервер хранит только публичный ключ и зашифрованную приватную часть.'),
      ...
      e('div',{class:'button-row security-key-actions'},
        ...
        identity&&unlocked&&!hasSystemUnlock&&e('button',{class:'secondary-button',disabled:busy,onClick:()=>void run(async()=>{await enableCollaborationSystemUnlock(user);setHasSystemUnlock(true);setStatus('Системная разблокировка E2EE-ключа включена на этом устройстве.');})},'Включить системную разблокировку'),
        identity&&hasSystemUnlock&&e('button',{class:'tertiary-button',disabled:busy,onClick:()=>{if(confirm('Забыть локальную системную разблокировку? Пароль аккаунта останется резервным способом.'))void run(async()=>{await forgetCollaborationSystemUnlock(user);setHasSystemUnlock(false);});}},'Забыть системную разблокировку')),
      identity&&recoveryRequired&&e('div',{class:'inline-alert warning recovery-panel'},e(UiIcon,{name:'warning',size:18}),e('div',null,e('strong',null,'Ключ требует восстановления после сброса пароля'),e('p',null,'Если ключ открыт на доверенном устройстве, перепривяжите его к текущему паролю. Иначе потребуется аварийная замена.'),
        unlocked&&e('form',{class:'recovery-form',onSubmit:(ev:Event)=>{ev.preventDefault();const secret=password;setPassword('');void run(async()=>{await persistCollaborationPasswordWrapper(user,secret);setRecoveryRequired(false);setStatus('Тот же E2EE-ключ безопасно перепривязан к текущему паролю.');});}},e('input',{type:'password',required:true,placeholder:'Текущий пароль аккаунта',value:password,onInput:(ev:Event)=>setPassword((ev.target as HTMLInputElement).value)}),e('button',{class:'primary',disabled:busy},'Перепривязать')))),
      identity&&e('details',{class:'danger-zone'},e('summary',null,'Аварийная замена E2EE-ключа'),e('div',{class:'danger-zone-body'},e('p',null,'Используйте только при утрате старого ключа. Личные E2EE-настройки напоминаний будут сброшены, а доступ к shared vault потребуется выдать заново.'),
        e('form',{class:'security-key-form',onSubmit:(ev:Event)=>{ev.preventDefault();const secret=password;setPassword('');if(!confirm('Создать новый E2EE-ключ? Старые member-envelopes станут недействительны.'))return;void run(async()=>{await replaceCollaborationIdentity(user,secret);setUnlocked(true);setStatus('Создан новый E2EE-ключ. Для shared vault потребуется regrant.');});}},e('label',{class:'field-stack'},e('span',null,'Текущий пароль'),e('input',{type:'password',required:true,value:password,onInput:(ev:Event)=>setPassword((ev.target as HTMLInputElement).value)})),e('button',{class:'danger-button',disabled:busy},'Заменить E2EE-ключ'))))));
}
