...
export function PasskeysScreen({user,onBack}:{user:User;onBack:()=>void}){
 ...
 async function remove(item:PasskeyInfo){if(!await appConfirm('Локальная системная разблокировка хранилищ управляется отдельно.',{title:`Удалить «${item.displayName}» как способ входа?`,confirmLabel:'Удалить',danger:true}))return;setBusy(true);setError('');try{await deletePasskey(item.id);await load();}catch(c){setError(authMessage(c));setBusy(false);}}
 return e('main',{class:'settings-screen passkeys-screen'},
  ...
  available&&!platform&&e(InfoTip,{label:'О поддержке ключей доступа'},'Браузер не подтверждает встроенный системный аутентификатор. Возможен внешний или синхронизируемый ключ доступа.'),
  ...
  e(InfoTip,{label:'Что происходит при удалении ключа'},'Удаление запрещает этому ключу вход в Tasks, но не удаляет его из iCloud Keychain, Google Password Manager или Windows и не меняет ключи шифрования хранилищ.'));
}
