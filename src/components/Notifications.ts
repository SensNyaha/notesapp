...
export function Notifications({user}:{user:User}){
  ...
  async function refresh(id=device){
    ...
    try{
      const cfg=await pushRequest(user,'config');
      const info=await pushRequest(user,'status',{deviceId:id});
      ...
      setConfig(cfg);setActive(info.active);setBrowserActive(Boolean(sub));setTest(info.test);setChecked(true);
  ...
  return e('section',{class:'card','aria-labelledby':'notifications-title'},
    ...
    e('p',{class:'muted'},'Тестовые уведомления для текущего устройства. Напоминания заметок будут добавлены следующим этапом.'),
