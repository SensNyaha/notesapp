...
export interface Revision { id: string; objectId: string; parent: string | null; sealed: Sealed; pending?: boolean }
export interface Vault { header: Header; key?: CryptoKey; pending?: boolean; deleted?: boolean; records: Revision[];
  transfer?: { target: string; revisions: string[] } }
export interface Stashed { id: string; sealed: Sealed; source: string }
export interface State { user: User; vaults: Vault[]; stash: Stashed[]; stashKey?: CryptoKey }
