...
Текущая рабочая версия: **0.20.10**, SQLite schema **14**. Сервер и PWA слушают порт **3100**. Полная карта документации находится в [`docs/README.md`](docs/README.md), фактическое покрытие функций — в [`docs/IMPLEMENTATION_STATUS.md`](docs/IMPLEMENTATION_STATUS.md), итоговый аудит экранов 01–54 — в [`docs/STAGE20_SCREEN_AUDIT.md`](docs/STAGE20_SCREEN_AUDIT.md).
...
Ожидаемый health текущей версии содержит `status: ok`, `version: 0.20.10`, `database: ok`, постоянный `installationId`, `bootCount` и время сервера.
